...
# 排序
# ==========================================================================
def _relevance_scores(papers: Sequence[Paper], topic: str, queries: Sequence[str]) -> List[float]:
    """用 TF-IDF 余弦相似度算文献与主题的相关性。"""
    if not papers:
        return []
    try:
    ...
    return [float(s) / hi for s in sims]
...
def rank_papers(
    ...
    rel = _relevance_scores(papers, topic, queries)
    ...
    for i, paper in enumerate(papers):
        ...
        paper["score"] = round(
            0.45 * rel[i] + 0.28 * cite_score + 0.17 * recency + 0.10 * coverage, 4
        )
...
def enrich_topn_fulltext(papers: List[Paper], settings: Optional[Settings] = None) -> int:
    """仅对 Top-N 高分文献拉全文，其余保持摘要模式（对应设计方案 6.1）。"""
    settings = settings or get_settings()
    if not settings.enable_fulltext or settings.top_n_fulltext <= 0:
        return 0
    targets = [p for p in papers[: settings.top_n_fulltext] if not p.get("fulltext")]
